<script setup>  
  ...
  const emit = defineEmits(['close', 'editable'])
</script>

<template>
  <base-dialog class="edit-element"  @close="emit('close')">
    <div style="display: flex;justify-content: center;">
      <header><p class="material-symbols-sharp icon-margin-header icon-font fill-font header-icon-layout">edit</p> Rendre {{ props.biover.name }} modifiable ou non modifiable</header>
    </div>
    <p class="description">{{ $t('TheMenu.Dialog.EditableDescription') }}</p>
