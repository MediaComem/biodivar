<script setup>
  ...
  const emit = defineEmits(['close', 'favoriAction'])
</script>

<template>
  <base-dialog class="edit-element"  @close="emit('close')">
    <div style="display: flex; justify-content: center;">
      <header><p class="material-symbols-sharp icon-margin-header icon-font fill-font header-icon-layout">star</p>{{ $t('TheMenu.Dialog.FavoriHeader_1') }} {{ props.biover.name }} {{ $t('TheMenu.Dialog.FavoriHeader_2') }}</header>  
    </div>
    <p class="description">{{ $t('TheMenu.Dialog.FavoriDescription') }}</p>
    ...
    <base-input class="dialog-input-color">
      <base-button class="editable" @click="emit('favoriAction', favoriState)">
